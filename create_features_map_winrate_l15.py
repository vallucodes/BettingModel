...
for prefix in ['team1', 'team2']:
    for m in maps:
        features_df[f'{prefix}_{m}_wr_l15'] = None

for team, team_match_group in team_matches.groupby('team_name'):
    team_match_group = team_match_group.sort_values('date')
    team_plays = team_map_rows[team_map_rows['team_name'] == team]
    for map_name in maps:
        ...
        for idx, match in team_match_group.iterrows():
            ...
            if prev_plays.empty:
                ...
                wr = prev_plays['map_won'].tail(15).mean()
            match_id = match['hltv_match_id']
            if features_df.loc[features_df['hltv_match_id'] == match_id, 'team1_name'].values[0] == team:
                features_df.loc[features_df['hltv_match_id'] == match_id, f'team1_{map_name}_wr_l15'] = wr
            else:
                features_df.loc[features_df['hltv_match_id'] == match_id, f'team2_{map_name}_wr_l15'] = wr

output_path = os.path.join(FEATURES_DIR, "features_map_winrate_l15.parquet")
