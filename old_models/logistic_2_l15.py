...
map_features_df = pd.read_parquet(f"{FEATURES_DIR}/features_map_winrate_l15.parquet")

wr_cols = [col for col in map_features_df.columns if '_wr_l15' in col]
...
for m in maps:
    map_features_df[f'{m}_wr_diff_l15'] = map_features_df[f'team1_{m}_wr_l15'] - map_features_df[f'team2_{m}_wr_l15']

df = df.merge(map_features_df[['hltv_match_id'] + [f'{m}_wr_diff_l15' for m in maps]], on='hltv_match_id', how='left')

feature_cols = [f'{m}_wr_diff_l15' for m in maps]
