...
for prefix in ['team1', 'team2']:
    for m in maps:
        features_df[f'{prefix}_{m}_wr_l5'] = None
    ...
    for map_name in maps:
        ...
        for idx, match in team_match_group.iterrows():
            ...
            if prev_plays.empty:
                ...
                wr = prev_plays['map_won'].tail(5).mean()
            match_id = match['hltv_match_id']
            if features_df.loc[features_df['hltv_match_id'] == match_id, 'team1_name'].values[0] == team:
                features_df.loc[features_df['hltv_match_id'] == match_id, f'team1_{map_name}_wr_l5'] = wr
            else:
                features_df.loc[features_df['hltv_match_id'] == match_id, f'team2_{map_name}_wr_l5'] = wr
